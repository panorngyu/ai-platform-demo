...
export const ocrService = {
    ...
    async recognizeInvoice(imagePath) {
        ...
        try {
            ...
            console.warn('[OCR] 发票识别失败，降级到 Mock:', error.message);
            return this.mockInvoice(imagePath);
        }
    },
    // Mock 发票识别
    mockInvoice(imagePath) {
        return {
            data: {
                InvoiceType: '增值税普通发票',
                InvoiceNum: '12345678',
    ...
    // 合同识别
    async recognizeContract(imagePath) {
        if (!isOCRConfigured()) {
        ...
        try {
            ...
            const text = (res.data.words_result || []).map((w) => w.words).join('\n');
            return {
            ...
            console.warn('[OCR] 合同识别失败，降级到 Mock:', error.message);
            return this.mockContract(imagePath);
        }
    },
    // Mock 合同识别
    mockContract(imagePath) {
        return {
            data: {

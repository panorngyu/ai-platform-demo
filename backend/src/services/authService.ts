...
export const authService = {
  ...
  async login(username: string, password: string): Promise<{ token: string; user: any }> {
    ...
    const token = jwt.sign(
      { userId: user.id, username: user.username, role: user.role, realName: user.realName },
      config.jwt.secret,
  ...
  // 企业微信 OAuth2 回调（降级处理）
  async wecomCallback(code: string): Promise<{ token: string; user: any }> {
    // 简化：返回 admin

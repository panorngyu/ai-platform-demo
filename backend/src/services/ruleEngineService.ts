...
export const ruleEngineService = {
  // 执行审核规则
  async executeRules(data: any, ruleType: string): Promise<any> {
    const rules = await this.getRules(ruleType)
    const engine = new Engine()

    rules.forEach((ruleData: any) => {
      const rule = new Rule({
    ...
    try {
      ...
      return {
        triggered: results.events.map((e: any) => ({
          action: e.type,
      ...
      console.warn('[RuleEngine] 规则执行失败:', (error as Error).message)
      return { triggered: [], allRules: rules.length, error: (error as Error).message, mock: !dbConnected }
    }
  },

  // 获取规则列表
  async getRules(type?: string): Promise<any[]> {
    if (dbConnected) {
      const where: any = { status: 'active' }
      if (type) where.type = type
      return await AuditRule.findAll({ where, order: [['priority', 'ASC']] })
    }
    if (type) return mockRules.filter((r) => r.type === type && r.status === 'active')
